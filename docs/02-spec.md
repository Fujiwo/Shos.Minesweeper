...
| 長押し | 押してから 400 ミリ秒以上、指やボタンを離さずに押し続けること（4.4 を参照） |
...
|---|------|------|----|
| 1 | 初級の盤面 | 9×9 | 3.1 |
| 2 | カスタムの範囲 | 幅 5〜30、高さ 5〜30、地雷 1〜（幅×高さ − 9） | 3.1 |
| 3 | 最初のクリックの安全保証 | 最初に開いたマスと、その周囲 8 マスには地雷を置かない | 3.3 |
| 4 | コード | 対応する。開いた数字のマスをクリック・タップして行う。左右同時クリックと中クリックは対象外 | 3.5 |
| 5 | ？マーク | 対応しない | 9 |
| 6 | タッチでの旗の立て方 | パターン D（長押しで旗。加えて旗モードの切り替えボタンを置く） | 4.2、4.3 |
...
| 11 | 記録と統計 | 初級・中級・上級のベストタイムだけを記録する | 3.8 |
| 12 | No Guessing モード | 対象外 | 9 |
| 13 | オフライン対応（PWA） | 対象外 | 9 |
| 14 | 公開先 | GitHub Pages のプロジェクト サイト（サブパスに置く） | 7 |
| 15 | キーボード操作 | 対応する | 4.5 |
...
| 経過時間 | 秒単位。最初にマスを開いたときに 0 から計り始め、勝敗が決まったら止める。999 で止まる |
...
- 初級、中級、上級のそれぞれで、勝利したときの最短の経過時間（秒）を記録する。カスタムは記録しない。
...
- 「開く」を開いた数字のマスに行うと、コード（3.5）になる。
- 右クリックでブラウザーのメニューが出ないようにする。
- 長押しが成立したら、指やボタンを離しても、クリック・タップの操作はしない（二重に操作しない）。
...
- 旗モードの間は、クリック・タップと長押しの役割を入れ替える（4.1 の表）。
- 旗モードは、リセットや難易度の変更をしても保つ。
- 旗モードかどうかが、盤面を見ている状態で分かるようにする（見た目は UI デザインで決める）。

### 4.4 長押しと誤操作の防止

- 長押しの判定時間は 400 ミリ秒とする。この値は実機での確認（工程 12、14）で調整してよい。
...
参考として、上級の盤面をスマートフォンの縦画面に表示すると、マスは 18〜24px 程度（下限にかかる端末ではスクロール）になる見込みである（調査書 7.4）。
...
- マスを開く、旗を立てるなどの操作から画面に反映されるまでを、上級の盤面で 100 ミリ秒以内とする（目標）。工程 12 と 14 の実機での確認で確かめる。
...
- スクリーンリーダーには、基本的な対応をする。
  ...
  - 各マスに、位置と状態が分かる名前を付ける（例: 「3 行 5 列、未開放」「3 行 5 列、2」）。
  - 勝利と敗北は、読み上げで知らせる。
...
- ブラウザーに保存するのは、ベストタイム（3.8）だけである。

## 7. 公開

- GitHub Pages のプロジェクト サイトとして公開する。URL はサブパス（`https://<ユーザー名>.github.io/Shos.Minesweeper/` の形）になるので、発行時に `<base href>` をサブパスに合わせる。
- 具体的な手順は、リリース準備（工程 13）で決める。

## 8. 受け入れ基準

リリースレビュー（工程 14）では、少なくとも次を確かめる。
