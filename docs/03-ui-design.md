...
| 工程 | 5. UI デザイン作成 |
| 作成日 | 2026-09-25 |
| 状態 | レビュー指摘を反映済み（docs/reviews/03-ui-design-review.md）。1.1.0 の改訂（見た目の洗練、効果音、演出）を 10 章に書いた（2026-09-26）。1.1.0 のレビューを待っている |
...
| 状態 | 形 |
|------|----|
| 未開放（A）、旗（E） | マスの四辺を 1px ずつ内側に縮めた、角の丸いタイル（半径はマスの大きさの 14%、最小 2px）。背景は、上端を `cell-closed` より 14% 白に寄せたグラデーション。上端に明るい線（白 28%）、下端に暗い線（黒 28%）を、4.2 の縁の太さで引く。タイルの間には、板の色（`board`）が 2px 見える |
...
- 残り地雷数（B）と経過時間（D）は、押せないことが分かるように、枠のない、沈んだ小さな面にする。背景は `page`、角の半径 10px、左右の余白 8px。アイコンは 18px にする（16px から）。数字の大きさ（20px）は変えない。
...
| 360〜399px | 最小 72px | 56px（左右の余白 6px） | 44px | 均等（最小 2px） | 4px |
| 360px 未満 | 最小 64px（文字 14px） | 52px（左右の余白 2px、アイコン 14px、数字 18px） | 44px | 均等（最小 2px） | 2px |
...
- 聞こえ方は、試聴用のページ（作業用のフォルダーの `sounds.html`。ブラウザーの Web Audio で、上の表のとおりに鳴らす）で確かめた。実装では、同じ設計を C# で合成する（アーキテクチャー設計で決める）。
...
| 勝ち | 地雷のマスの旗（自動で立てた旗を含む）が、マスの 20% だけ上に跳ねて戻る（300 ミリ秒） | 300 ミリ秒 × d ÷ dmax | 600 ミリ秒 |
