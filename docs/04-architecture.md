...
| 工程 | 7. アーキテクチャー設計書作成 |
| 作成日 | 2026-09-25 |
| 状態 | レビュー指摘を反映済み（docs/reviews/04-architecture-review.md）。クラス設計で改めた点を反映済み（docs/reviews/05-class-design-review.md）。工程 12 で、WPF 版・コンソール版と共有する部品のプロジェクトを加えた（docs/reviews/code-review.md の工程 12 の R5）。この変更を含めてレビューをやり直し、指摘を反映した（docs/reviews/04-architecture-review.md の「再レビュー」）。リファクタリングのやり直し（docs/reviews/code-review.md の RR4）で、`InputMapping` の名前を `PressMapping` に改めた。1.1.0 の改訂（効果音、演出、ツールバーと勝利カードの置き方）を行った（2026-09-26）。1.1.0 のレビューを待っている |
...
1. 効果音の規則と合成を、新しいクラスライブラリ `Shos.Minesweeper.SoundEffects` に置く。Web 版と WPF 版が参照し、コンソール版は参照しない（4 章、5 章）。
...
| 効果音は、使うアプリだけが参照する（1.1.0） | 効果音の定義（どの出来事でどの音を鳴らすか、音そのもの）は、Web 版と WPF 版で共有し、コンソール版は使わない（CLAUDE.md の「目的」）。そこで、Presentation とは別のクラスライブラリ（SoundEffects）に置く。コンソール版が音に依存しないことを、参照しないことで、コンパイラーに守らせる |
...
| 15 | 効果音の規則 | 操作の結果から、鳴らす音を決める（1 回の操作で 1 つだけ、勝敗が決まったら勝ちか負けの音だけ、何も起きなければ鳴らさない） | 仕様 5.6 | SoundEffects: `SoundEffectMapping` |
| 16 | 効果音の合成 | 6 つの音の波形を作る | 仕様 5.6、UI 10.6 | SoundEffects: `SoundEffectSynthesizer` |
...
├─ Shos.Minesweeper.SoundEffects/     クラスライブラリ（net10.0）。効果音の規則と合成（1.1.0）。Web 版と WPF 版が参照し、コンソール版は参照しない
│   ├─ SoundEffect.cs、SoundEffectMapping.cs   操作の結果から鳴らす効果音
│   └─ SoundEffectSynthesizer.cs      効果音の波形の合成
...
├─ Shos.Minesweeper.SoundEffects.Tests/  SoundEffects のテスト（xUnit。1.1.0）
...
flowchart LR
    ...
    SoundEffectsTests["Shos.Minesweeper.SoundEffects.Tests"] --> SoundEffects["Shos.Minesweeper.SoundEffects"]
    AppProject --> Presentation
    AppProject --> SoundEffects
    Presentation --> GameLogic
    SoundEffects --> GameLogic
...
**効果音の部品を別のプロジェクト（SoundEffects）にする理由（1.1.0）**

- 効果音の定義（どの出来事でどの音を鳴らすか、音そのもの）は、Web 版と WPF 版で共有し、コンソール版は使わない（CLAUDE.md の「目的」）。
- Presentation に置く案も考えた。プロジェクトは増えないが、Presentation はコンソール版も参照するので、コンソール版が使わない音の部品を抱え、使っていないことを人が見張り続けることになる。別のプロジェクトにすれば、コンソール版は参照しないだけで音に依存しなくなる。GameLogic を別のプロジェクトにした理由と同じく、依存の向きをコンパイラーで守れる。
- Web アプリに置く案も考えた。WPF 版で同じ規則と合成を書き直すことになり、2 つの版の音がずれていく。
- SoundEffects は GameLogic（操作の結果）だけに依存し、Presentation には依存しない。SoundEffects が作るのは音の波形（サンプルの並び）までで、鳴らすのは各アプリである。Web 版は Web Audio で鳴らす（9.4）。WPF 版の再生の方法（調査書 8.8）は、WPF 版の一巡で決める。
- テストは、共有するほかのプロジェクトと同じく、別のテストプロジェクト（`SoundEffects.Tests`）にする。Web アプリをビルドせずに流せる。
...
- `SoundEffects` は、仕様書の「効果音」をそのまま名前にした（1.1.0）。音楽（BGM）などは作らないので（仕様書 9 章）、`Audio` のような広い名前にしない。
...
flowchart TB
    ...
    subgraph SoundEffectsProject["Shos.Minesweeper.SoundEffects"]
        Sound["SoundEffect・SoundEffectMapping・SoundEffectSynthesizer"]
    end
...
| Presentation | GameLogic、.NET の基本ライブラリ | Blazor、WPF、JavaScript、SoundEffects、アプリのすべて |
| SoundEffects（1.1.0） | GameLogic（操作の結果）、.NET の基本ライブラリ | Blazor、WPF、JavaScript、Presentation、アプリのすべて |
...
| Browser | GameLogic（`BestTimes` とその保存の形式）、SoundEffects（効果音の種類と合成。1.1.0）、`IJSRuntime` | コンポーネント |
| Pages・Components | 上のすべて | — |

- 矢印は一方向で、逆向きの依存（GameLogic や Presentation がアプリを知る、Input がコンポーネントを知る）はない。
- Presentation と SoundEffects は、互いに依存しない。コンソール版は GameLogic と Presentation だけを参照すればよい（1.1.0）。
...
| 操作の結果（1.1.0。型の名前はクラス設計で決める） | 1 回の盤面の操作で起きたこと。何も起きなかった、開いた（新たに開いたマスの一覧）、旗を立てた、旗を外した、のどれかと、その操作で勝敗が決まったか（勝ち・負け） | 鳴らす音、演出（決めるのは SoundEffects とアプリ） |
...
**SoundEffects（Web 版と WPF 版で共有する効果音の部品。1.1.0）**
...
| 直前の操作（操作したマスと操作の結果。1.1.0） | `GamePage` | 操作を行うのが `GamePage` で、盤面が演出に使う。新しいゲームを始めたら消す |
...
- **演出のために描き直しを増やさない。** 演出は、操作の後の 1 回の描き直しで、マスに CSS のクラスと変数を付けるだけにする。動きは CSS のアニメーションが進め、C# のタイマーで描き直さない。
...
- 演出の途中で次の操作をしたら、直前の操作が新しい操作に置き換わる。前の操作の演出のクラスは外れ、そのマスはすぐに最後の見た目になる（14 章の決定 9）。
...
- そこで `browser.js` は、`document` で `keydown`、`pointerdown`、`pointerup`、`touchend` を捕捉の段階（Blazor の処理より前）で受け、そのたびに、`AudioContext` がまだなければ作り、動いていなければ（`suspended`、iOS の `interrupted`）`resume()` する。電話などで止まった後も、次の操作で鳴るようになる。
...
| SoundEffects（1.1.0） | xUnit | 操作の結果から決まる効果音を、入力と期待値の表で確かめる。合成した波形は、長さ、最大の振幅、始まりと終わりが 0 に近いこと（雑音が出ないこと）、同じ入力から同じ波形になること、を確かめる |
...
- 1.1.0 で加えるテストプロジェクトは、ソリューションに加えれば、ワークフローの `dotnet test` で流れる。効果音はプログラムで合成するので、公開するファイルは増えない。
...
| 6 | 効果音を鳴らせる状態にするのは JavaScript の側で、利用者の操作と見なされるイベントのたびに行う | 調査書 8.7。Blazor の処理の中では、操作の最中と見なされないおそれがある。iOS で止まった `AudioContext` も、次の操作で動かせる |
...
| 9 | 演出の途中で次の操作をしたら、前の操作の演出は終える（すぐに最後の見た目にする） | 演出するのは直前の操作だけにする。演出を重ねると、同時に動くマスが増えて何が起きたかが分かりにくくなり、描き方も複雑になる。仕様書 5.7 の「演出の間も操作を受け付ける」は満たす |
...
| 効果音を鳴らすインターフェイス（Web 版と WPF 版の再生を同じ形にする抽象。1.1.0） | 鳴らし方はアプリごとに違い、共有すべき部分（鳴らす音を決める、波形を作る）は SoundEffects にある。Web 版の実装は 1 つで、テストは JavaScript interop の偽物で確かめる |
| WAV の形式への変換（1.1.0） | Web 版では要らない（波形をそのまま渡す。9.4）。WPF 版の一巡で要ると分かったときに、SoundEffects に足す |
...
1.1.0 で確認する点（回答がなければ、前例に従い、推す案どおりに確定する）:

| 点 | 推す案 | 見送る案 |
|----|--------|----------|
| 効果音の部品の置き場所 | 新しいクラスライブラリ `Shos.Minesweeper.SoundEffects` と、そのテストプロジェクトを加える（プロジェクトは 7 つから 9 つになる）。コンソール版が音に依存しないことを、コンパイラーで守れる（4 章） | Presentation に置く。プロジェクトは増えないが、コンソール版が使わない音の部品を抱える |
