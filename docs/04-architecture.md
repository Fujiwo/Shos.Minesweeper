...
3. 共通のゲームの進め方（`GameSession`。Presentation）が `Game` に操作を伝え、操作の結果から鳴らす効果音を決めて、外から受け取った「音の出口」に渡す。出口を渡さない版（コンソール版）では鳴らない。`BoardView` は、直前の操作から演出を描く。演出の動きと時間は CSS が受け持つ（6.2、8.6、9.2。ユーザーの決定）。
...
| 効果音の部品は共有し、鳴らし方は各アプリが持つ（1.1.0） | 効果音の部品（どの出来事でどの音を鳴らすか、音の波形）は、UI の技術に依存しない C# で書けるので、Presentation に置いて Web 版と WPF 版で共有する。Presentation は、効果音の実装（ブラウザーの Web Audio、WPF の再生の方法やライブラリ）に依存しない。共通のゲームの進め方は効果音をサポートし、鳴らす効果音を「音の出口」に渡す。出口の中身は各アプリが渡し、渡さなければ鳴らない。共通の側は、どの版かを知らない（ユーザーの決定） |
...
| 差し替え口は、テストが求めるものと、版ごとに中身が違うものだけ | 時刻、乱数（地雷の配置）は、テストで固定する必要があるので差し替えられるようにする。音の出口（1.1.0）は、版ごとに中身が違う（鳴らさない版がある）ので差し替えられるようにする。それ以外の抽象（インターフェイスなど）は作らない（15 章） |
...
関心事 5〜9、13〜16、18、20 は画面に関わるが、計算と判定だけで、画面の部品を必要としない。そこで Razor コンポーネントから切り離し、ふつうの C# のクラスにする。コンポーネントは、これらのクラスの結果を描くことと、イベントを渡すことだけを受け持つ。
...
- **共通のゲームの進め方が、効果音をサポートする。** `GameSession`（6.2）が、操作の結果から鳴らす効果音を決め、「音の出口」に渡す。出口の形（効果音を 1 つ受け取るデリゲート）は Presentation が決め、中身は各アプリが渡す。Web 版は Web Audio で鳴らす出口を、WPF 版は WPF の方法で鳴らす出口を渡す。コンソール版は出口を渡さず、既定の「何もしない出口」が使われるので、鳴らない（ユーザーの決定）。
- 共通の側には、「コンソール版なら」という分岐も、`#if` も、「音を鳴らせるか」を表す値もない。常に出口を呼ぶだけで、鳴るかどうかは出口の中身で決まる（何もしない既定の出口は、いわゆる Null Object である）。Presentation は出口の形だけを知り、中身（効果音の実装）には依存しない。
- 出口をインターフェイスではなくデリゲートにするのは、操作が「効果音を鳴らす」の 1 つだけだからである。`Game` が地雷を置く場所の選び方を、省略できるデリゲート（`MineChooser`）で受け取るのと同じ形にそろえる。
...
flowchart TB
    ...
    subgraph PresentationProject["Shos.Minesweeper.Presentation"]
        Shared["DifficultyNames・Announcements・PressMapping・<br/>SoundEffectMapping・SoundEffectSynthesizer・GameSession など"]
    end
...
- Presentation は、効果音の実装に依存しない。音の出口の形を決めるのは Presentation で、中身を渡すのは各アプリである（依存の向きは、アプリから Presentation へのまま）。コンソール版は GameLogic と Presentation を参照し、`GameSession` に音の出口を渡さない（1.1.0）。
...
| `GameSession` | 1 回のゲームの進め方。現在の `Game` を持ち、新しいゲームを始め、盤面の操作（開く・旗）を `Game` に伝える。操作の結果から `SoundEffectMapping` で鳴らす効果音を決め、音の出口に渡す。何かが起きた操作を、直前の操作（操作したマスと操作の結果）として覚え、新しいゲームを始めたら消す。どの版かを知らない |
| 音の出口（型の名前はクラス設計で決める） | 効果音を 1 つ受け取るデリゲート。`GameSession` を作るときに渡し、省略すると何もしない出口になる（鳴らない）。呼ばれたらすぐに返す（鳴り終わるのを待たない） |

- `GameSession` が持たないもの: ベストタイム、読み上げの文、効果音のオンとオフ、旗モード、盤面の置き方。
  - 効果音のオンとオフは、鳴らす実装がある版だけの事情なので、音の出口の中身（Web 版では `SoundEffectPlayer`）が持つ。`GameSession` に置くと、コンソール版に意味のない値が残る。
  - ベストタイムの記録と保存、読み上げの文は、1.1.0 の目的（効果音）に要らないので、今回は移さない。保存は版ごとに方法が違う（Web 版は localStorage への非同期の書き込み）。WPF 版・コンソール版の一巡で、3 つの版の流れを見てから、`GameSession` に移すかを決める（CLAUDE.md の「目的」の前提）。
- `GameSession` は、`Game` に渡す時刻（`TimeProvider`）と地雷を置く場所の選び方も受け取る。テストで盤面と時刻を決めるためである（12 章）。
...
| `SoundEffectPlayer`（1.1.0） | Browser | Web 版の音の出口の中身。効果音の波形を `SoundEffectSynthesizer` で合成して、`BrowserFeatures` で JavaScript に渡しておき、求められた効果音を鳴らす。効果音のオンとオフを持ち、オフなら鳴らさない |
...
1.1.0 で `GamePage` が受け持つことが増えたので、`GamePage` は判断を持たず、つなぐだけにする。操作と鳴らす効果音は `GameSession`（その中で `SoundEffectMapping`）、演出は `BoardAnimation`、盤面の置き方は `BoardPlacement`、保存は `BestTimeStorage` と `SoundSettingStorage` が決める。`GamePage` に残るのは、誰の結果を誰に渡すかと、状態の持ち主としての値だけである。
...
| 登録するもの | 有効期間 | 使う側 |
|--------------|----------|--------|
| `TimeProvider`（本番は `TimeProvider.System`） | シングルトン | `GamePage`（`Game` を作るときに渡す。1.1.0 からは `GameSession` に渡し、`GameSession` が `Game` を作る）、`BoardView`（`PressGesture` に渡す）、`ElapsedTime`（経過時間を確かめるタイマー） |
...
- `GameSession` も DI に登録せず、`GamePage` が作る（1.1.0）。時刻には DI の `TimeProvider` を、音の出口には `SoundEffectPlayer` の鳴らすメソッドを渡す。地雷を置く場所の選び方は渡さず、GameLogic の既定（乱数）を使う。
...
| 状態 | 持ち主 | 理由 |
|------|--------|------|
| 現在のゲーム（`Game`） | `GameSession`（1.1.0。`GameSession` は `GamePage` が持つ） | ツールバーと盤面の両方が表示に使う。1.1.0 から、新しいゲームを作るのは `GameSession` である |
| 現在の難易度 | `GameSession`（現在の `Game` の難易度。1.1.0） | リセットで同じ難易度を使い、ダイアログにも渡す |
...
| 効果音のオンとオフ（1.1.0） | `SoundEffectPlayer` | 鳴らす実装がある版だけの事情なので、音の出口の中身が持ち、オフなら鳴らさない。ツールバーのボタンは、`GamePage` を通してこの値を表示し、切り替える。ページを開いたときに、`GamePage` が保存から読んで設定する |
| 直前の操作（操作したマスと操作の結果。1.1.0） | `GameSession` | 操作を行うのが `GameSession` で、盤面が演出に使う。何かが起きた操作のときだけ置き換える（何も起きなかった操作では、前の演出を止めない）。新しいゲームを始めたら消す |

- ゲームの状態を持つのは `Game` だけにする。コンポーネントは `Game` の状態を読んで描くだけで、コピーを持たない。直前の操作は、ゲームの状態ではなく、演出のための記録なので、`Game` ではなく `GameSession` が持つ（6.1）。
...
flowchart LR
    ...
    GamePage -- "GameSession（1.1.0）・BestTimes を呼ぶ" --> Logic["Presentation・GameLogic"]
    GamePage -- "描き直し（状態を引数で渡す）" --> Child
```

- `Game` から画面へ知らせる仕組み（イベントなど）は作らない。Blazor は、イベントを処理した後にコンポーネントを描き直すので、描き直しのときに `Game` の状態を読めば足りる。操作の結果（1.1.0）も、知らせる仕組みではなく、`GameSession` が呼んだ操作の戻り値として受け取る。1.1.0 から、`GamePage` は盤面の操作を `GameSession` を通して行う。
...
- 1.1.0 からは、`GamePage` は `GameSession` を通して `Game` を開く（8.6）。
...
sequenceDiagram
    ...
    GamePage->>GS: 開く（8.6）
    Note over GamePage: Game の状態が勝利になった
...
sequenceDiagram
    ...
    participant SEP as SoundEffectPlayer（音の出口）
    participant BA as BoardAnimation
    User->>BV: タップ
    BV->>GamePage: 開く意図（盤面の位置）
    GamePage->>GS: 開く
    GS->>Game: 開く
    Game-->>GS: 操作の結果（新たに開いたマス 23 個、勝敗は決まっていない）
    GS->>SEM: 鳴らす効果音
    SEM-->>GS: 連鎖
    GS->>SEP: 連鎖（音の出口を呼ぶ）
    Note over SEP: オンなら browser.js が Web Audio で鳴らす（9.4）。<br/>オフなら何もしない。どちらもすぐ返る
    Note over GS: 何かが起きた操作なので、直前の操作（操作したマスと結果）として覚える
    Note over GamePage: 勝敗が決まっていれば、勝利カードと読み上げを出す（8.4）
    GamePage-->>BV: 描き直し（Game と直前の操作を渡す）
    BV->>BA: 演出するマス
    BA-->>BV: マスごとの種類と遅れの比
...
- 長押しで行った操作の音も、この流れで `GameSession` が音の出口に渡す。`BoardView` は、今までどおり振動だけを受け持つ（8.3）。長押しの成立そのものの音はない（仕様書 4.4）。
...
- コンソール版では、同じ `GameSession` が同じ流れで動き、音の出口を渡していないので、鳴らないだけである。
...
| Presentation の効果音の部品と `GameSession`（1.1.0） | xUnit（`Presentation.Tests`） | `GameSession` は、渡された効果音を覚えるだけの偽物の出口を渡し、操作ごとに渡った効果音（何も起きなければ渡らない）と直前の操作を確かめる。出口を渡さないときも、同じ操作が例外なく進むことを確かめる。操作の結果から決まる効果音を、入力と期待値の表で確かめる。合成した波形は、長さ、最大の振幅、始まりと終わりが 0 に近いこと（雑音が出ないこと）、同じ入力から同じ波形になること、を確かめる |
...
| 音の出口（1.1.0） | Web 版は `SoundEffectPlayer`。コンソール版は渡さない（鳴らさない） | 渡された効果音を覚える偽物 | 版ごとに中身が違うため（要求）。テストでは、どの操作でどの効果音を渡したかを確かめる |
...
| 10 | 長押しで行った操作の効果音は、ほかの操作と同じく `GameSession` が音の出口に渡す | 仕様書 4.4 のとおり、長押しの成立そのものの音はない。操作の結果から音を決める流れを 1 つにできる |
| 11 | 効果音のオンとオフは、音の出口の中身（Web 版の `SoundEffectPlayer`）が持つ | 設定は、鳴らす実装がある版だけの事情である。共通の `GameSession` に置くと、コンソール版に意味のない値が残る |
| 12 | 音の出口は、呼ばれたらすぐに返す（鳴り終わるのを待たない） | 盤面の操作の応答（仕様書 6.2）を遅らせない。出口を待つ形にすると、版ごとの鳴らし方の遅さが共通の流れに入り込む |
...
| 音の出口のインターフェイス（1.1.0） | 操作が「効果音を鳴らす」の 1 つだけなので、デリゲートで足りる。`Game` が地雷の選び方を受け取る形（`MineChooser`）にそろえる |
| `GameSession` へのベストタイムと読み上げの移動（1.1.0） | 1.1.0 の目的（効果音）に要らない。WPF 版・コンソール版の一巡で、3 つの版の流れを見てから決める（6.2） |
| 音を鳴らせるかを表す値や、版を見分ける分岐（1.1.0） | 何もしない既定の出口で足りる。共通の側は、どの版かを知らない |
...
| `Game` の「直前の操作」の状態（1.1.0） | ルールには要らない、演出のための記録である。`Game` は操作の結果を戻り値で返し、覚えるのは `GameSession` である（6.1、6.2） |
...
| 効果音の合成にかかる時間（1.1.0）。WebAssembly の .NET は、ネイティブより遅い。最初の描画の後の 1 回だが、スマートフォンで長いと、最初の操作の応答を待たせる。長いときは、サンプリング周波数を下げるか、効果音ごとに初めて鳴らすときに合成する | 工程 11 で計った。PC の Chrome で 6 つを合わせて 20〜30 ミリ秒（docs/reviews/code-review.md の 1.1.0 区切り 4）。工程 13 で実機で確かめる |
| C# から渡したバイト列を `Float32Array` として読めるか（1.1.0）。`Uint8Array` の先頭の位置が 4 バイトの境界にないと、そのままでは読めない。複写してから読めば避けられる | 工程 11 で、JavaScript の側で複写してから読む形にし、ブラウザーで鳴ることを確かめた（区切り 4） |
| C# で合成した音と、試聴のページ（docs/sounds-preview.html。Web Audio の部品で鳴らす）の音が、同じに聞こえるか（1.1.0）。とくに負けの音の雑音とフィルター | 工程 11 で波形を数値で比べた（区切り 2。音程のある音はほぼ同じ、負けの音は雑音の乱数の違いだけ）。耳での聞き比べは工程 13 |
...
| 共通のゲームのロジックが、効果音をサポートすること（レビューの途中で、ユーザーが決めた） | **Presentation の `GameSession` が、鳴らす効果音を音の出口に渡す。出口は外から受け取り、既定は何もしない。共通の側はどの版かに依存しない**（4 章、6.2） | 各アプリが、鳴らす効果音を決めて自分で鳴らす（設計書の前の版）。共通の側は判断だけを持つが、アプリが鳴らし忘れても気づけない |
