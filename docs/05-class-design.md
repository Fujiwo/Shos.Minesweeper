...
- 雑音の低域通過は、Web Audio の `BiquadFilterNode`（`lowpass`）と同じ式（Audio EQ Cookbook の低域通過の式。Q は Web Audio の既定の 1 で、Web Audio の仕様どおりデシベルとして読む）で、サンプルごとに境の周波数を変えて計算する。試聴のページと同じ音にするためである。
