...
## コードの現状
実装（工程 11）は、クラス設計書（docs/05-class-design.md）の 8 章の区切り 1〜7 をすべて終えた。区切りごとのレビューと、実際のブラウザーで確かめたことは docs/reviews/code-review.md にある。工程 12（結合テスト）で確かめることも、そこの「まだ確かめていないこと」にまとめてある。文書の進み具合は「開発手順」の「現在の工程」を見ること。
